...
- **How the units were settled — against the book, by attribution:** the venue stamps a post-trade snapshot's `transactTime` **byte-identically** to the trade's `tradeTime`, so a print's consequence in the book is *identified*, not merely adjacent to it. Worked example, entirely from the recorded public tape: a print on `ewc-pres-bra-2026-10-04-flabol` at `price 0.2800` carrying `quantity.value 5.0000`, `tradeTime 2026-07-29T01:56:27.328744187Z`. Exactly one snapshot for that market carries that same timestamp; the resting bid at 0.2800 holds 240.0000 across the five snapshots before it and reads 235.0000 on it and after — consumed by **exactly the printed `value`**. Nothing else can account for it: that is the only trade on that market all day. Book sizes are contracts — `qty` is the bare decimal the venue also accepts as a bare integer `quantity` on `POST /v1/orders` — so the printed `value` is contracts too. USD notional would have had to remove 5.0000/0.28 = 17.86 from that level, and it did not.
- **How far that generalizes:** sweeping one full day's tape (2026-07-29) for every print whose post-trade snapshot is identified this way, **thousands match contracts and ZERO match notional**. The exact count depends on how you treat prints sharing a snapshot (3,113 contract matches across 278 traded markets by the sweep used for this entry); the zero does not move. A non-integral `value` is **not** evidence against contracts — PM-US positions are genuinely fractional (see `pmus-positions-*`), and 1,899 of those matches carry a fractional size (`51.4800`, `1.0500`) agreeing to the cent with an equally fractional level.
