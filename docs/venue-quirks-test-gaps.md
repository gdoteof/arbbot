...
- `kalshi-fill-channel-deltas-not-cumulative` — partial: `fills::kalshi_tests` pins delta accumulation, `trade_id` dedupe across BOTH sources, that the total is identical whether or not the venue replays (`venue_truth_survives_a_replay_because_it_carries_trade_ids`), and that a foreign id space is REFUSED rather than double-counted (`a_foreign_id_space_is_refused_rather_than_double_counted`, with `an_order_the_socket_never_saw_still_reconciles` pinning that the guard does not fire on the case the repair exists for); `tests/order_path.rs` pins the row shape, both envelopes, cursor paging in either sort order, the truncated-history error and Background priority. **GAP, and it is a LIVE probe not a unit test**: nothing establishes that WS `trade_id` and REST `trade_id` are the same identifier, because no raw WS fill frame has ever been captured — the guard above contains that risk but does not answer it. Suggested: a standalone READ-ONLY subscriber to the `fill` channel that reconnects on a timer and logs every raw frame for N seconds after each `subscribe` — places no orders, needs no new fill, and settles the replay question as a side effect.
...
- `pmus-ws-trade-quantity-is-money-wrapped-contracts` — pinned: `rust/bins/arb-recorder/src/pmus.rs::trade_size_is_the_contract_count_inside_the_money_wrapper` (the units, against a real tape frame and the book depth it consumes), `::a_trade_field_of_an_unknown_shape_is_dropped_not_stringified` and `::a_book_level_of_an_unknown_shape_is_dropped_not_stringified` (the general guard on this feed), `rust/bins/arb-recorder/src/core.rs::dec_string_takes_scalars_and_refuses_composites` (the helper), `::a_refused_composite_is_counted_after_the_one_log_line_is_spent` (the drop stays visible on the `[hb]` line after the once-per-process log is spent) and `::the_shape_preview_bounds_a_payload_without_splitting_a_char` (the diagnostic cannot panic on a non-ASCII payload), plus the same assertion on the other two feeds in `kalshi.rs` and `pmintl.rs`.
...
- Entries: 46 — Kalshi 17 (9 order + 8 market data), Polymarket US 17 (9 order + 3 positions + 5 market data), Polymarket intl 5, cross-venue 7.
- Fully pinned: 19. Partial: 11. Full GAP: 16. (Every partial carries at least one untested load-bearing edge listed above.)
